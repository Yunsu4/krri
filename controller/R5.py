...
import pandas as pd
from model.Line1_4_riders_ratio import Line1_transfer,Line4_transfer
...
# 입력된 시간대에 해당하는 환승 인원
    ...
    #시간대별 환승 인원
    line1 =Line1_transfer(date, dow)
    line4 =Line4_transfer(date, dow)


    #입력된 시간대에 맞는 환승 인원 추출
 ...
 # 상하행선 혼잡도 비율
    ...
    # 환승 인원 구하기
    # 상하행선의 혼잡도 비율과 각 호선별 상하행 혼잡도 비율의 리스트를 zip으로 묶는다. 
    # 그리고 반복문을 시행한다. start=1로 하여 인덱스가 원래 0이지만 1부터로 변경된다.
    for i, (ratio, car_ratio) in enumerate(zip([a, b, c, d], [upCar_avg_1, downCar_avg_1, upCar_avg_4, downCar_avg_4]), start=1):
        if i ==1:
            Line1_upLine_transfer = calculate_transfer_counts(line1_transfer, ratio, car_ratio, i) # 1호선 상행선 환승 인원 계산
        elif i ==2:
            Line1_downLine_transfer = calculate_transfer_counts(line1_transfer, ratio, car_ratio, i) # 1호선 하행선 환승 인원 계산
        elif i ==3:
            Line4_upLine_transfer = calculate_transfer_counts(line4_transfer, ratio, car_ratio, i) # 4호선 상행선 환승 인원 계산
        else:
            Line4_downLine_transfer = calculate_transfer_counts(line4_transfer, ratio, car_ratio, i) # 4호선 하행선 환승 인원 계산

    R5_1=pd.DataFrame() # 결과를 담을 R5_1 데이터프레임 생성
    R5_2=pd.DataFrame() # 결과를 담을 R5_2 데이터프레임 생성

    R5_1 = pd.merge(Line1_upLine_transfer, Line1_downLine_transfer, on=['hour','dow'], how='outer').fillna(0) # 합치기, null 값은 0으로 채우기
    R5_2 = pd.merge(Line4_upLine_transfer, Line4_downLine_transfer, on=['hour','dow'], how='outer').fillna(0) # 합치기, null 값은 0으로 채우기

    R5 = pd.merge(R5_1, R5_2, on=['hour','dow'], how='outer').fillna(0) # 합치기, null 값은 0으로 채우기
    R5 = R5.drop(columns=['hour','dow'], axis=1) # hour, dow 컬럼은 필요없으므로 삭제
    ...
    R5.columns=[str(col) if col != 0 else '환승인원' for col in R5.columns] # 원래 index였던 0이 컬럼이 되므로 0 대신 환승인원이라는 컬럼 이름을 갖도록 수정



 # 모든 시간대의 환승 인원을 더하여 요일 평균과 동일한지 확인
...
def calculate_transfer_counts(LineTransfer, ratio, car_ratio, column_prefix):

    car_ratio = car_ratio.fillna(0) # null 값은 0으로 채우기

    updated_transfer = pd.DataFrame() # 연산 결과를 담을 데이터프레임 생성
    ...
    for i in range(10):
            ...
            if col_name in car_ratio.columns:
                updated_transfer[col_name] = LineTransfer * ratio * car_ratio[col_name]*0.01


    # 이후 데이터프레임 merge를 위해 hour, dow 컬럼 추가
